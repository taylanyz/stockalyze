...
def test_summarize_marj_ve_borc_trendi():
    # marj %10 -> %15 (iyileşiyor), borç/özkaynak 1.0 -> 0.5 (iyileşiyor)
    h = _hist(
        _p(2022, 100, 10, 100, 100, ocf=9),
        _p(2023, 120, 15, 130, 90, ocf=13),
        _p(2024, 150, 30, 200, 100, ocf=28),
    )
    lines = " ".join(summarize(h))
    assert "Satış" in lines
    assert "iyileşiyor" in lines            # marj VEYA borç
...
def test_tek_yil_yetersiz():
    ...
    assert summarize(h) == ["Trend çıkarımı için yeterli yıllık veri yok."]
